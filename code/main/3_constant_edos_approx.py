...
def heatmap(*, save_name: str | None = "auto") -> None:
    ...
    if save_name is not None:
        filename = "stage_3_edos_vs_const_edos_heatmap.svg" if save_name == "auto" else save_name
...
def rel_T(T: float, *, save_name: str | None = "auto") -> None:
    ...
    set_figure_title(fig, title)
    if save_name is not None:
        filename = f"stage_3_rel_error_T{int(round(T))}K.svg" if save_name == "auto" else save_name
        save_svg(fig, filename)
    plt.show()
