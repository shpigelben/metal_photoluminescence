...
# Rebuilding Rosei's Interband $\epsilon_2$ of Gold in His Own Notation

> [!info] How this note is organised
> Equations are tagged with **GRW's own numbering**; a prime (e.g. $(4')$) marks my suggested correction of a printed equation. Tags (11)–(12) continue past the paper's ten: they are the fully assembled integral and its flattened numerical form, which GRW never display. The two callout styles are: blue **Remark** notes for asides, and red **warning** boxes for the three printed equations that look like typos.

## Introduction

This note rebuilds Rosei's interband $\epsilon_2(\hbar\omega,T)$ of gold from Fermi's Golden Rule, in his own notation, so that the ten numbered equations of Guerrisi, Rosei & Winsemius ("GRW", *Phys. Rev. B* **12**, 557 (1975)) all come out of one derivation. A few things worth flagging before we start.

*Where it goes.* Everything feeds two equations, GRW's (7) and (9). Eq. (7) is the thermally weighted, energy-integrated joint density of states,
