...
Inserting the determinant, $D=\det(M)$, of this transformation as the Jacobean, the measure in the new variables becomes
$$
d\mathcal{E}d\Delta \to D dudv
$$

where $D$ is the determinant of the transformation. To find $v(\mathcal{E},\Delta)$ which appears in the denominator of the previous Jacobean, we calculate the inverse transformation
$$
...
From the inverse transformation we extract $u(\mathcal{E},\Delta)$ and $v(\mathcal{E},\Delta)$. 
...
Before we insert $v(\mathcal{E},\Delta)$ into the complete Jacobean we find the integration limits in the new variable by imposing the constraints $u>0$ and $v>0$, this results in the following energy bounds
$$
\begin{align}
...
We can also now write $v(\mathcal{E}, \Delta)$ neatly
$$
v(\mathcal{E}, \Delta) = \frac{\bar{A}}{D}\Big(\mathcal{E}_{max}(\Delta)-\mathcal{E}\Big)
$$
and insert it into the transformation. The measure in the new energy variable becomes
$$
