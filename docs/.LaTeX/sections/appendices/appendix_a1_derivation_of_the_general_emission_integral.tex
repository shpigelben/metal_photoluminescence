...
\begin{itemize}
  \item \textbf{nonlinear effects for strong laser pumps \ pulsed dynamics? Well, for pulsed radiation $\mathbf{A}(\hat{\mathbf{r}},t)$ would be different).}
  \item Coulomb gauge - always have this freedom?
  \item Need to also account for phonon part in Hamiltonian for intraband transitions?
\end{itemize}
...
\label{eq:first_order_interaction}
\end{align}
...
\label{eq:second_order_transition_rate}
\end{equation}
...
\label{eq:continuum_single_rate}
\end{align}
